...
def _ensure_browser():
        ...
        import shutil
        # Check if chromium binary exists anywhere under the playwright cache
        cache_dir = os.path.expanduser("~/.cache/ms-playwright")
        found = False
        if os.path.isdir(cache_dir):
                    for root, dirs, files in os.walk(cache_dir):
                                    if "chrome-headless-shell" in files or "chromium" in files:
                                                        found = True
                                                        break
                                            if not found:
                                                        subprocess.run(
...
def run_scrape() -> list:
    ...
    return asyncio.run(scrape_all_errors(session_state))
